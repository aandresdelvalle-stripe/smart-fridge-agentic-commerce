...
export const buildMarketplaceApp = async (overrides: Partial<Pick<MarketplaceDependencies, "payments">> = {}) => {
  ...
  const dependencies: MarketplaceDependencies = {
    checkouts: new CheckoutStore(),
    payments: overrides.payments ?? new StripePaymentGateway(),
    auditLog,
  };
  await registerMarketplaceRoutes(app, dependencies);
